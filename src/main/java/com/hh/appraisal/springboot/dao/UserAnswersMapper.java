...
public interface UserAnswersMapper extends BaseMapper<UserAnswers> {
	...
	@Select("	SELECT\n"
			+ "	ROUND( SUM( QUESTION_OPTIONS.DIVISOR_VALUE ) / COUNT( * ),1) AS DIVISOR_AVERAG, DIVISOR.CODE,\n"
			+ "	DIVISOR.DIVISOR_NAME,DIVISOR.DIVISOR_DESC  FROM USER_ANSWERS,QUESTION_OPTIONS,QUESTION, DIVISOR \n"
			+ "WHERE\n"
			+ "	USER_ANSWERS.QUESTION_CODE = QUESTION.CODE \n"
			+ "	AND USER_ANSWERS.QUESTION_OPTIONS_CODE = QUESTION_OPTIONS.CODE \n"
			+ "	AND QUESTION.DIVISOR_CODE = DIVISOR.`CODE`  \n"
			+ "	AND USER_ANSWERS.EVALUATION_USER_CODE = #{EVALUATOION_CODE} \n"
			+ "	AND USER_ANSWERS.IS_COMPLETE = 'Y' \n"
			+ "GROUP BY\n"
			+ "	DIVISOR.CODE,\n"
			+ "	DIVISOR.DIVISOR_NAME,\n"
			+ "	DIVISOR.DIVISOR_DESC  ORDER BY divisor.create_time")
	List<DivisorAverageBean> getDivisorListInfo2ByXlcp(@Param("EVALUATOION_CODE") String EVALUATOION_CODE);
	...
	@Select("SELECT COUNT(1) from USER_ANSWERS where EVALUATION_USER_CODE=#{EVALUATOION_CODE} AND IS_COMPLETE='Y' ")
	int getCompleteQuestionCount(@Param("EVALUATOION_CODE") String EVALUATOION_CODE);
	...
	@Select("SELECT"
			+ "	SUM( QUESTION_OPTIONS.DIVISOR_VALUE ) AS tot_musk "
			+ "FROM\n"
			+ "	USER_ANSWERS,\n"
			+ "	QUESTION_OPTIONS,\n"
			+ "	QUESTION \n"
			...
			+ "	AND USER_ANSWERS.EVALUATION_USER_CODE = #{EVALUATOION_CODE} \n"
			+ "	AND USER_ANSWERS.IS_COMPLETE = 'Y' ")
	long getTotMusk(@Param("EVALUATOION_CODE") String EVALUATOION_CODE);
	
	@Select("SELECT ROUND( SUM( QUESTION_OPTIONS.DIVISOR_VALUE ) / COUNT( * ), 2 ) AS avg_musk "
			+ "FROM\n"
			+ "	USER_ANSWERS,\n"
			+ "	QUESTION_OPTIONS,\n"
			+ "	QUESTION \n"
			...
			+ "	AND USER_ANSWERS.EVALUATION_USER_CODE = #{EVALUATOION_CODE} \n"
			+ "	AND USER_ANSWERS.IS_COMPLETE = 'Y' ")
	double getAvgMusk(@Param("EVALUATOION_CODE") String EVALUATOION_CODE);
	...
	@Select("SELECT\n"
			...
			+ "	AND USER_ANSWERS.EVALUATION_USER_CODE = #{EVALUATOION_CODE} \n"
			+ "	AND USER_ANSWERS.IS_COMPLETE = 'Y'\n"
			+ "	AND  QUESTION_OPTIONS.divisor_value>=2")
	long getMoreThan2QuestionNum(@Param("EVALUATOION_CODE") String EVALUATOION_CODE);
